...
world's largest supercomputer including the examples shown here.
...
\item[Lock Interactive Render] Specify a pause between an interactive
  render and a still render.  This pause allows you to let go of the mouse
  and drag again (to perhaps change from rotate to pan) without having to
  wait for a full still render.
\item[Use outline for LOD rendering] Normally ParaView uses a decimated
  version of the geometry for its interactive render.  When this box is
  checked, ParaView instead uses a simple bounding box outline.  This saves
  the time to make the decimated geometry and any rendering time.
\item[Allow Rendering Interrupts] \index{rendering interrupts} When
  ...
  interactive render.  Not all rendering modes support interrupts.
...
\item[Coincident Topology Resolution] \index{coincident geometry} It is
  sometimes the case you wish to render lines that are coincident with a
  surface (for example, rendering a wireframe of cells on top of their
  surface).  Rendering systems generally can have problems resolving hidden
  surfaces in these circumstances.  These options allow you to change the
  ``tricks'' ParaView's rendering uses to resolve these issues.  Generally
  you do not need to change these options unless there is a problem with
  the rendering.
...
\item[Remote Render Threshold] \index{remote render threshold} The checkbox
  ...
  (usually the client).  Unchecking this box forces all geometry to be
  copied to the display, which can be dangerous.
...
\item[Apply presets for...] The optimal settings used on this page are
  dependent on the network characteristics.  This chooser lets you
  automatically set up the settings for several common networks.
\item[Non-Interactive Subsample Rate] Tiled displays are often used for
  multiple things.  For example, the display may be used for small
  collaborations or during large presentations.  In the large presentation,
  the audience is unlikely to be able to resolve all of the pixels in the
  display.  If that is the case, this option allows you to subsample the
  still renders and save a significant amount of time.
...
\item If the network connection has a high latency, adjust the parameters
  ...
  Threshold} and try checking the \gui{Use outline for LOD rendering} box
  if necessary.
\item If still render is slow, try turning up the \gui{Lock Interactive
  Render} value to avoid unnecessary renders.
\end{enumerate}
