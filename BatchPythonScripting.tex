...
The most convenient way to do so is to rely on the way that \pyfunc{Render} returns a view, creating one first if necessary.
If you prefer, you can create specific view types via \pyfunc{CreateView(`viewname')} or \pyfunc{CreateRenderView}, \pyfunc{CreateXYPlotView} and the like.
However you make them, call \pyfunc{GetRenderView} to get a list of all Views,  or \pyfunc{GetActiveView} get access to the currently active view
