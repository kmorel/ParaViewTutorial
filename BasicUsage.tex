...
\begin{multicols}{2}
  ...
  \item raw image files (.nrrd)
  \item LANL cosmology data (.cosmo)
  \item netcdf POP Ocean files
  \item VPIC (.vpc)
...
\parpic[r]{\includegraphics[width=1in]{images/FiltersMenu}}
...
\parpic[r]{\includegraphics[width=2.5in]{images/QuickLaunch}}
...
\begin{exercise}{Plot Series Display Options}
  ...
  \begin{inlinefig}
    \includegraphics[width=3in]{images/LinePlot4}
  \end{inlinefig}
...
slow for large data. \footnote{If you are willing to wait, enable the
\gui{Rescale to Temporal Range} button on the edit color scale dialog
box~\icon{pqEditColor24} and ParaView will compute this overall temporal
range automatically.}
Second, when you animate over time, it is important
...
dialog. Launch this dialog from the \gui{Edit} menu. From the dialog,
enter the characteristics of the data that you are searching for. For
example, points whose velocity ``VEL`` magnitude is between 1500
and 1600.  Click the \gui{Run Selection Query} and ParaView will
create the selection for you and display the results, annotated with
the color and optionally labels chosen via the dialog box.

\begin{inlinefig}
  \includegraphics[width=4in]{images/FindData}
\end{inlinefig}
