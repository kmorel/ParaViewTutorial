...
the \gui{Source} menu.  Sources can be used to add annotation to a view,
but they are also very handy when exploring ParaView’s features.  Let us
start with a simple one.  Go to the \gui{Source} menu and select
\gui{Cylinder}.  Once you select the \gui{Cylinder} item you will notice
that an item named \gui{CylinderSource1} is added to and selected in the
pipeline browser.  You will also notice that the object inspector is filled
with the properties for the cylinder source.  Click the \gui{Apply} button
\apply to accept the default parameters.

Once you click \gui{Apply}, the cylinder object will be displayed in the 3D
...
more.  Notice that the \gui{Apply} button \apply has turned green (or blue
...
that you are viewing.  Hitting the \gui{Apply} button will accept these
changes whereas hitting the \gui{Reset} button \reset will revert the
options back to the last time they were applied.  Hit the \gui{Apply}
...
redo~\icon{pqRedo32} buttons in the toolbar, which are new to ParaView 3.
...
are also special undo camera~\icon{pqUndoCamera24} and redo
camera~\icon{pqRedoCamera24} buttons.  These allow you to go back and forth
...
Also be aware of the \gui{Display} tab in the object inspector.  This tab
...
object by selecting the \gui{Properties} tab and hitting delete \delete in
...
one off of the \gui{File} menu, and there is also toolbar button for
...
Let us open our first file now.  Click the \gui{Open} toolbar (or menu item) and
open the file \gui{disk\_out\_ref.ex2}.  Note that opening a file is a two step
...
Play with the data representation a bit.  Use the variable chooser to color
the surface by the \gui{Pres} variable.  Then turn the color legend on to
see the actual pressure values.  To see the structure of the mesh, change
the representation to \gui{Surface With Edges}.  You can view both the cell
structure and the interior of the mesh with the \gui{Wireframe}
representation.
...
\parpic[r]{\includegraphics[width=2in]{images/FiltersMenu}}
...
Let us apply our first filter.  Make sure that \gui{disk\_out\_ref.ex2} is
selected in the pipeline browser and then select the contour
filter~\contour from the filter toolbar or \gui{Filters} menu.  Notice that
...
parameter to the \gui{Temp} variable.  Then, change the isosurface value to
\gui{400}.  Finally, hit \apply.  You will see the isosurface appear inside
...
\gui{ExtractSurface1} object is not connected directly to the
\gui{disk\_out\_ref.ex2}, then this is what went wrong.  If so, you can
