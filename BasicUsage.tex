...
\item[Properties Panel] \index{properties panel} The properties panel
  ...
  object.  The properties are by default coupled with an
  \keyterm{Information} tab that shows a basic summary of the data produced
...
\begin{exercise}{Creating a Source}
  ...
  If you scroll down to the bottom of the properties panel, you will notice
...
\index{display properties}
As discussed in Exercise~\ref{ex:CreatingASource},
another location for rendering options is the \gui{Display} section in the
properties panel.  These properties provide the rendering options that
apply specifically for the selected object.  It includes the visibility,
coloring, and representation.  By default many of the lesser used display
properties are hidden.  The \keyterm{advanced properties}
toggle~\icon{pqAdvanced26} can be used to show or hide these extra
parameters.  Be aware that some of the view options and object display
options can be repeated elsewhere in the ParaView GUI for convenience.
...
\begin{exercise}{Modifying Rendering Parameters}
  ...
  in the pipeline browser.  In the properties panel, scroll down to the
  \gui{Display} section.

  \begin{itemize}
  \item \index{solid color} \index{object color} Set the (solid) color of
    the cylinder by clicking the \gui{Edit}~\icon{pqEditColor24} button.
  \item \index{cube axis} \index{axis!cube|see{cube axis}} Show 3D axes
    with tic marks giving spatial position (make \gui{Cube Axis} visible).
  \item \index{shiny} \index{specular intensity} Make the cylinder shiny.
    (Hint: Make the \gui{Specular Intensity} 1.0.  This is an advanced
    parameter, so you will need to toggle \icon{pqAdvanced26}.)
  ...
  \index{delete~button} delete \delete in the properties panel.
\end{exercise}
...
over 130 distinct file formats, and the list grows as more types get
...
\item[\plotGlobal Plot Global Variables Over Time] Data sets sometimes
  ...
  informaiton over time.  ParaView's handling of time is described in
  Section~\ref{sec:Time}.
\item[\plotOverLine Plot Over Line] Allows you to define a line in segment
...
\begin{inlinefig}
  \includegraphics[width=0.8\scw]{images/ColorScaleEditorSimple}
\end{inlinefig}

When you first open up the color scale editor, you get this simplified
interface.  From here you can change the scaling of the the colors.  You
can also choose one of the many pre-defined color scales by pressing the
\gui{Choose Preset} button.

However, you can also have much finer controls over the transfer function.
To access these, press the \icon{pqAdvanced26} toggle in the upper right
corner of the color scale editor dialog box.

\begin{inlinefig}
  \includegraphics[width=\scw]{images/ColorScaleEditor}
\end{inlinefig}

The colorful box at top displays the colors of the transfer function and
the one below that plots the transparency.\footnote{For surface rendering,
  the transparency controls are hidden unless ``Enable Opacity Function''
  is enabled.}  The dots on the transfer functions represent the
\keyterm{control points}.  The control points are the specific color and
opacity you set at particular scalar values, and the colors and
transparency are interpolated between them.  Clicking on a blank spot in
either bar will create a new control point.  Clicking on an existing
control point will select it.  The selected control point can be dragged
throughout the box to change its scalar value and transparency (if
applicable).  Double clicking on a color control point will allow you to
change the color.  The selected control point will be deleted when you hit
the backspace or delete key.

Directly below the respective color and transparency bars are text entry
widgets to numerically specify the \gui{Scalar Value} or
\gui{Opacity}\index{opacity} of the selected control point.  The
\gui{Scale} parameter adjusts the unit length of the opacity calculation.
Larger numbers make the volume less opaque.  The \gui{Color
...
clicking the \gui{Save} button.  The \gui{Choose Preset} button brings up a
...
\begin{exercise}{Modifying Volume Rendering Transfer Functions}
  ...
  \item Change the volume rendering to be more representative of heat.
    ...
    \gui{Black-Body Radiation} in the dialog box, and then click \gui{Close}.
  \item Turn on the advanced options by pressing the \icon{pqAdvanced26}
    toggle.
  \item Try adding and changing control points and observe their effect on
    the volume rendering.  Click the \gui{Apply} button or turn on the
    \gui{Render View Immediately} checkbox to see the effects of your
    change.
  \end{enumerate}
...
\item Open the edit color scale dialog box~\icon{pqEditColor24} and specify
  a range for the data.  This is a good choice if you cannot find, or do
  not know, a ``representative'' time step or if you already know a good
  range to use.  You change the range of your data by unselecting
  \gui{Automatically Rescale to Fit Data Range}, then click on \gui{Rescale
    Range} and fill in the \gui{Minimum} and \gui{Maximum} entries.
...
\begin{exercise}{Save Animation}
  ...
  \index{Ogg/Theora} is used in many open source viewers.  Otherwise,
...
\begin{exercise}{Plot Over Time}
  ...
  \item If you do not have a few cells selected from the previous exercise,
    select a few now. (For this exercise it is not a good idea to select a
    large amount of cells.)
  \item With the selection still active, add the Plot Selection Over
